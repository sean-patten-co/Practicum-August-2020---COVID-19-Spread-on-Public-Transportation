...
LandData <- read_excel("C:/Users/snlpt/Desktop/MSDS 692/Datas/LandData.xls")
...
####Create_Empty_Dates
#Function will create a data frame of dates where the observed number of cases and deaths are zero.
#This will be joined in remove_cumulative to ensure a complete data set (zero observed cases in each month where there were no cases reported)
Create_Empty_Dates <- function(){

  missing_dates <- 
date <- as.Date(c("2020-01-01", "2020-02-01", "2020-03-01", "2020-04-01", "2020-05-01", "2020-06-01", "2020-07-01"))
cases <- c(0,0,0,0,0,0,0)
deaths <- c(0,0,0,0,0,0,0)

missing_dates <- data.frame(date, cases, deaths)
...
#groups are passed into this function and then the cumulative totals are removed by month. 
...
#  x <- us_counties_covidcounts %>% group_by(fips)  
  ...
  while(i<=nrow(x)){ #100 for testing nrow(x) for real run
      if (i==1){
  ...
  #replace NA's in deaths and cases
  
  is.na(new_cases$cases) <- 0
  is.na(new_cases$deaths) <- 0
  
  #remove incomplete and return
  
  new_cases <- new_cases %>% rbind(Create_Empty_Dates())
  
  return(new_cases[complete.cases(new_cases), ])
  
}

#End Remove Cumulative



##################

#FIPS data needs to be in character form with five digits. This changes the FIPS codes from numeric to character and adds leading zeroes.
...
#The length of time under lockdown is suspected to have an influence on the number of cases. This finds the number of days between the start and stop dates of
#stay at home orders. 
...
#create identifiers that are linkable to one another
#different data sets have different identifiers, some have only county and state, some have FIPS some have Urban Areas.
#We need to create a way to link these identifiers together. 
#This process will drop certain counties from the dataset because they are not included in an Urban Area. 
...
full_data_set <- right_join(full_data_set, urban_area_to_county[,c("FIPS", "UANAME")], by = "FIPS")

full_data_set <- right_join(full_data_set, LandData[c("FIPS", "CAREALAND")], by = "FIPS")
...
####create and export a covid count dataset to ensure the data is kept safely unchanged
####commented out since we have a dataset
...
#write.csv(uscntscvdcnts, file = "C:/Users/snlpt/Desktop/MSDS 692/Datas/uscntscvdcntsb.csv") #section above takes several minutes to run export to save time

#rm(uscntscvdcnts)

#######End uscntscvdcnts

###import the data produced by the above. 

uscntscvdcnts <- read_csv("C:/Users/snlpt/Desktop/MSDS 692/Datas/uscntscvdcntsb.csv")
uscntscvdcnts <- uscntscvdcnts[,-1]

#######################


uscntscvdcnts <- uscntscvdcnts %>% group_by(fips) %>% group_modify(~ aggregate(cbind(cases)~month(date),data=.,FUN=sum)) #find the number of new cases by FIPS

#rm(uscntscvdcnts) #remove the dataset if is corrupted
...
uscntscvdcnts_cases <- uscntscvdcnts %>% spread('month(date)', cases) %>% select(-"7")

names(uscntscvdcnts_cases) <- c("FIPS","V1.cases", "V2.cases", "V3.cases", "V4.cases","V5.cases","V6.cases")

uscntscvdcnts_cases <- uscntscvdcnts_cases %>% ungroup() %>% mutate(V1.cases = coalesce(V1.cases, 0), V2.cases= coalesce(V2.cases, 0), V3.cases = coalesce(V3.cases, 0), V4.cases = coalesce(V4.cases, 0), V5.cases = coalesce(V5.cases, 0), V6.cases = coalesce(V6.cases, 0), FIPS = FIPS)
...
#panel_final <- read.xlsx("final_data_assemble08012020.xlsx", "Sheet1")

###

panel_final <- panel_final %>% melt(c("FIPS","ctystid","CAREALAND", "Years.of.Potential.Life.Lost.Rate","POPESTIMATE2018","Restaurants","Avg_PMiles_Trip","SRVC_AREA_SQML","SRVC_AREA_POP","Pass_Miles","U_P_Trips"), c("JAN_C", "FEB_C", "MAR_C", "APR_C", "MAY_C", "JUN_C"))

namethis <- names(final_data_assemble)

namethis <- namethis[c(1:8,10:15,20:24)]

panel_final <- melt(final_data_assemble[,namethis], id.vars = namethis[c(1,8:18)])
...
policy_by_county <- policy_by_county %>% difference_days()

final_data_assemble <- final_data_assemble %>% left_join(policy_by_county, by = "FIPS") #add days at home in final_data_assemble

panel_final <- panel_final %>% left_join(policy_by_county, by = "FIPS")
...
write.xlsx(final_data_assemble, file = "final_data_assemble01132021.xlsx") #needs days at home

#write.csv(panel_final, file = "panel_final_10282020.csv")

#write.xlsx(policy_by_county, file = "policy_by_county10282020.xlsx")
