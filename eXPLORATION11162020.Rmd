...
county_data_ <- read.xlsx("C:/Users/snlpt/Desktop/MSDS 692/datas/final_data_assemblenovfin.xlsx", sheetName = "Sheet1") #changed to import new file 11/05/2020
...
#Replace infinite data with county population density

county_data_$SRVC_Pop_Dense[is.infinite(county_data_$SRVC_Pop_Dense)] <- county_data_$SRVC_Pop_Dense
...
#create cases per person by monthn
...
summary(panel_final$V2.cases)

plot_histogram(panel_final$V2.cases)

summary(panel_final$V1.cases)

plot_histogram(panel_final$V1.cases)
...
test_modr <- plm(cases ~ Restaurants + Avg_PMiles_Trip + POPESTIMATE2018 + V4.rides, data = panel_final, model="random", effect = "time", index = c("FIPS", "variable")) 

summary(test_modr)

test_mod <- plm(cases ~ FIPS, model="within", effect = "time", index = c("FIPS", "variable"), data=panel_final)

summary(test_mod)

is.pbalanced(panel_final)

cases_per_mod <- plm(Cases_per_Person ~ Res_per_Person  + Years.of.Potential.Life.Lost.Rate + Avg_PMiles_Trip + POPESTIMATE2018 +  FIPS + days_at_home  + V4.rides, data = panel_final, model="random", effect = "time", index = c("FIPS", "variable"))
...
bayes_Mar_ <- bayes.lm(V3.cases ~  Restaurants + RperPerson + POPESTIMATE2018 + Years.of.Potential.Life.Lost.Rate + SRVC_Pop_Dense + POPESTIMATE2018 + Pop_Dense + Pass_Miles + V2.cases + V1.cases + V4.rides + (SRVC_Pop_Dense*Pop_Dense) + (Pop_Dense*Pass_Miles), data = county_data_)
...
covid_cases_rf3 <- randomForest(Cases_per_Person ~ Restaurants   + YPLLR + Avg_PMiles_Trip + POPESTIMATE2018 + days_at_home + + V1.rides + V2.rides + V3.rides + V4.rides + U_P_Trips + Pop_Dense + FIPS + V1.cases + V2.cases + V3.cases + V4.cases + V5.cases + V6.cases, data = panel_final, ntree = 100, mtry = 10, importance = TRUE)
...
B <- county_data_$Jan_Cases_per_Person
