...
itsdone <- function(){
  send.mail(from = "snlpttn@gmail.com",
            to = c("Sean Patten <sean.l.patten@gmail.com>"),
            subject = "Code is done running",
            body = "Test emails body",
            smtp = list(host.name = "smtp.gmail.com", port = 465, 
                        user.name = "snlpttn@gmail.com",            
                        passwd = "1f0QbGmAoFwa", ssl = TRUE), 
            authenticate = TRUE,
            send = TRUE)
}
...
#This process will drop certain counties from the dataset because they do not include an urban area. 
...
panel_final <- panel_final %>% melt(c("FIPS","ctystid","CAREALAND", "Years of Potential Life Lost Rate","POPESTIMATE2018","Restaurants","Avg_PMiles_Trip","SRVC_AREA_SQML","SRVC_AREA_POP","Pass_Miles","U_P_Trips"), c("V1.cases", "V2.cases", "V3.cases", "V4.cases", "V5.cases", "V6.cases"))

namethis <- names(final_data_assemble)

namethis <- namethis[c(1:8,10:15,20:24)]

panel_final <- melt(final_data_assemble[,namethis], id.vars = namethis[c(1,8:19)])
...
write.xlsx(final_data_assemble, file = "final_data_assemblenovfin.xlsx")

write.csv(panel_final, file = "panel_final_novfin.csv")

write.xlsx(policy_by_county, file = "policy_by_countynovfin.xlsx")
